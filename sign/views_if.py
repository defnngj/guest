...
def get_event_list(request):
    ...
    if eid != '':
        ...
        try:
            ...
            return JsonResponse({'ststus':200, 'message':'success', 'data':event})
...
def user_sign(request):
    ...
    if n_time >= e_time:
        return JsonResponse({'ststus':10024,'message':'event has started'})

    result = Guest.objects.filter(phone = phone)
    if not result:
        return JsonResponse({'ststus':10025,'message':'user phone null'})

    result = Guest.objects.filter(phone = phone,event_id = eid)
    if not result:
        return JsonResponse({'ststus':10026,'message':'user did not participate in the conference'})

    result = Guest.objects.get(phone = phone).sign
    if result:
        return JsonResponse({'ststus':10027,'message':'user has sign in'})
    else:
